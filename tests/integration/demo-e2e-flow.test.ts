...
describe('End-to-End Demo Flow: seed → register → search → hire → pay → verify', () => {
  ...
  it('should complete full demo lifecycle via API', async () => {
    ...
    expect(healthBefore.body.version).toBe(require('../../package.json').version);
    ...
    expect(demoStatus.body.version).toBe(require('../../package.json').version);
    ...
    expect(agentCard.body.version).toBe(require('../../package.json').version);
