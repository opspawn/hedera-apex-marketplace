...
describe('Sprint 25: Homepage Polish', () => {
  ...
  test('homepage includes Try Agent Chat CTA', async () => {
    const res = await req(app, '/');
    expect(res.text).toContain('Try Agent Chat');
  ...
  test('displays version 0.28.0', async () => {
    const res = await req(app, '/');
    expect(res.text).toContain('v0.28.0');
  });
  ...
  test('GET /health returns version 0.28.0', async () => {
  ...
  test('GET /api/stats returns version 0.28.0', async () => {
