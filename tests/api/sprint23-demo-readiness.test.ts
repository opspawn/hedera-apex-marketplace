...
describe('Sprint 23: /.well-known/agent.json', () => {
  ...
  test('agent.json has version 0.24.0', async () => {
    const res = await request(app, 'GET', '/.well-known/agent.json');
    expect(res.body.version).toBe('0.24.0');
  });

  test('agent.json has protocols array with 6 standards', async () => {
    const res = await request(app, 'GET', '/.well-known/agent.json');
    expect(res.body.protocols).toHaveLength(6);
...
describe('Sprint 23: Version bump', () => {
  ...
  test('health returns v0.24.0', async () => {
    const res = await request(app, 'GET', '/health');
    expect(res.body.version).toBe('0.24.0');
  });

  test('stats returns v0.24.0', async () => {
    const res = await request(app, 'GET', '/api/stats');
    expect(res.body.version).toBe('0.24.0');
  });

  test('test_count is 1550', async () => {
    const res = await request(app, 'GET', '/health');
    expect(res.body.test_count).toBe(1550);
  });

  test('ready endpoint returns v0.24.0', async () => {
