...
describe('Sprint 23: /.well-known/agent.json', () => {
  ...
  test('agent.json has version 0.23.0', async () => {
    const res = await request(app, 'GET', '/.well-known/agent.json');
    expect(res.body.version).toBe('0.23.0');
  });
...
describe('Sprint 23: Version bump', () => {
  ...
  test('health returns v0.23.0', async () => {
    const res = await request(app, 'GET', '/health');
    expect(res.body.version).toBe('0.23.0');
  });

  test('stats returns v0.23.0', async () => {
    const res = await request(app, 'GET', '/api/stats');
    expect(res.body.version).toBe('0.23.0');
  });

  test('test_count is 1500', async () => {
    const res = await request(app, 'GET', '/health');
    expect(res.body.test_count).toBe(1500);
  });

  test('ready endpoint returns v0.23.0', async () => {
